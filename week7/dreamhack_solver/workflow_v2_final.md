...
- **차별화 포인트**: trivy + cve_lookup 조합으로 다른 도구들이 놓치는 의존성 취약점 발견
...
* **도구**: `dreamhack_solver:file_analysis`
* **문제 유형 판별**:
...
* **제한된 도구**: `dreamhack_solver:curl`, `dreamhack_solver:python_exec` (requests/session만)
* **탐색 전략**:
...
**4단계**: CVE 무기화 (PoC 조회 및 실전 변환)
* **사고**: "CVE 정보를 실제 공격으로 변환하는 우리만의 강점을 발휘하자"
* **도구**: `dreamhack_solver:cve_lookup`
* **PoC 분석 전략**:
  - "이 CVE를 우리 타겟 환경에 어떻게 적용할까?"
  - "1-2단계에서 수집한 정보와 어떻게 결합할까?"
  - "페이로드를 어떻게 커스터마이징해야 할까?"
* **에러 처리** (재시도 최대 3회, 0.5초 간격):
  - **도구 실패**: "CVE 조회 도구 실행 실패" → 5단계로 이동 (`next_strategy: "use_trivy_results"`)
  - **CVE 정보 부재**: "CVE 상세 정보 없음" → 5단계로 이동 (`next_strategy: "general_web_attacks"`)
  - **PoC 추출 실패**: "실행 가능한 PoC 없음" → 5단계로 이동 (`next_strategy: "hybrid_approach"`)
* **성공**: "CVE PoC 획득 완료" → 5단계 진행 (`next_strategy: "cve_priority"`)
...
* **동적 전략 결정** (이전 단계 `next_strategy` 기반):
  ...
  **use_trivy_results**: 도구 실패했지만 3단계 정보 활용
...
* **도구**: `dreamhack_solver:curl`, `dreamhack_solver:python_exec`
* **3계층 재시도 구조**:
...
```json
{
  ...
  "execution_summary": {
    ...
    "tools_used": ["file_analysis","curl","trivy","cve_lookup","python_exec"],
    "retry_count": 2,
...
### **dreamhack_solver:file_analysis**
- **사용 단계**: 1단계
- **목적**: 로컬 파일 분석 및 취약점 발견
...
### **dreamhank_solver:python_exec**
- **사용 단계**: 2단계, 6단계  
- **목적**: 복잡한 HTTP 요청 처리 (requests/session 활용)
- **활용**: 세션 관리, 복잡한 페이로드 전송, 응답 파싱
...
### **dreamhack_solver:cve_lookup**
- **사용 단계**: 4단계
- **목적**: CVE 상세 정보 및 PoC 조회
- **입력**: CVE ID 리스트, 타겟 기술 정보
- **출력**: CVE 상세정보, 실행가능한 exploit 코드
